...
(function ( $, window, document, undefined ) {
    ...
    Plugin.prototype.init = function () {
		...
		this.bgAlign = this.$element.attr('data-bg-align');

		// These the are the alignment positions
		this.xPos = '50%';
		this.yPos = '50%';

		if ( this.direction === 'up' || this.direction === 'down' ) {
			if ( this.bgAlign === 'left' ) {
				this.xPos = '0';
			} else if ( this.bgAlign === 'right' ) {
				this.xPos = '100%';
			}
		} else if ( this.direction === 'left' || this.direction === 'right' ) {
			if ( this.bgAlign === 'top' ) {
				this.yPos = '0';
			} else if ( this.bgAlign === 'bottom' ) {
				this.yPos = '100%';
			}
		}
    ...
    Plugin.prototype.scrolly2 = function() {
		...
		// don't perform parallax for video backgrounds to save performance
		if ( this.$element.find('[id^=video-]').length > 0 ) {
		...
		if ( this.direction === 'up' || this.direction === 'down' ) {
			if ( this.bgAlign === 'left' ) {
				this.xPos = this.$element.offset().left + 'px';
			} else if ( this.bgAlign === 'right' ) {
				this.xPos = 'calc(100% - ' + ( $(window).width() - this.$element.offset().left - this.$element.width() ) + 'px)';
			}
		} else if ( this.direction === 'left' || this.direction === 'right' ) {
			if ( this.bgAlign === 'top' ) {
				this.yPos = '0';
